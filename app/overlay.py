...
# MediaPipe hip landmark indices for distance indicator
L_HIP, R_HIP = 23, 24
...
def draw_position_guide(
    ...
    """Draw a human silhouette guide showing the user where to stand.

    Only visible while the buffer is filling (buf_len < 60).
    Fades out once the buffer is full.
    """
    if buf_len >= 60:
        return frame
    ...
    # build silhouette on a separate layer for transparency
    guide = frame.copy()
    color = (200, 200, 200)
    thickness = 2

    # head
    head_y = cy - int(h * 0.28)
    head_r = int(h * 0.05)
    cv2.circle(guide, (cx, head_y), head_r, color, thickness, cv2.LINE_AA)

    # neck to shoulder line
    neck_y   = head_y + head_r
    shoulder_y = neck_y + int(h * 0.04)
    shoulder_w = int(w * 0.12)
    cv2.line(guide, (cx, neck_y), (cx, shoulder_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx - shoulder_w, shoulder_y),
             (cx + shoulder_w, shoulder_y), color, thickness, cv2.LINE_AA)

    # arms
    elbow_y  = shoulder_y + int(h * 0.12)
    wrist_y  = elbow_y + int(h * 0.12)
    arm_w    = int(w * 0.18)
    # left arm
    cv2.line(guide, (cx - shoulder_w, shoulder_y), (cx - arm_w, elbow_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx - arm_w, elbow_y), (cx - int(arm_w * 1.1), wrist_y), color, thickness, cv2.LINE_AA)
    # right arm
    cv2.line(guide, (cx + shoulder_w, shoulder_y), (cx + arm_w, elbow_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx + arm_w, elbow_y), (cx + int(arm_w * 1.1), wrist_y), color, thickness, cv2.LINE_AA)

    # torso
    hip_y = shoulder_y + int(h * 0.24)
    hip_w = int(w * 0.08)
    cv2.line(guide, (cx - shoulder_w, shoulder_y), (cx - hip_w, hip_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx + shoulder_w, shoulder_y), (cx + hip_w, hip_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx - hip_w, hip_y), (cx + hip_w, hip_y), color, thickness, cv2.LINE_AA)

    # legs
    knee_y = hip_y + int(h * 0.16)
    foot_y = knee_y + int(h * 0.16)
    leg_w  = int(w * 0.04)
    # left leg
    cv2.line(guide, (cx - hip_w, hip_y), (cx - leg_w, knee_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx - leg_w, knee_y), (cx - leg_w, foot_y), color, thickness, cv2.LINE_AA)
    # right leg
    cv2.line(guide, (cx + hip_w, hip_y), (cx + leg_w, knee_y), color, thickness, cv2.LINE_AA)
    cv2.line(guide, (cx + leg_w, knee_y), (cx + leg_w, foot_y), color, thickness, cv2.LINE_AA)

    # blend silhouette onto frame — more transparent as buffer fills
    alpha = 0.25 + 0.20 * (1.0 - buf_len / 60.0)
    cv2.addWeighted(guide, alpha, frame, 1.0 - alpha, 0, frame)

    # labels
    cv2.putText(frame, "STAND HERE",
                (cx - 60, head_y - head_r - 14),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, COLOR_WHITE, 2, cv2.LINE_AA)
    cv2.putText(frame, "Align your body with the outline",
                (cx - 140, foot_y + 24),
                cv2.FONT_HERSHEY_SIMPLEX, 0.45, COLOR_GREY, 1, cv2.LINE_AA)
...
def draw_distance_indicator(
    ...
    """Draw step-back / step-forward arrows based on which landmarks are missing."""
    ...
    hips_visible      = vis[L_HIP] >= 0.5 and vis[R_HIP] >= 0.5
    shoulders_visible = vis[L_SHOULDER] >= 0.5 and vis[R_SHOULDER] >= 0.5

    if not hips_visible and shoulders_visible:
        # hips cut off — user too close, step back
        _draw_arrow(frame, (w // 2, h - 30), direction="down",
                    label="Step back", color=COLOR_YELLOW)
    elif not shoulders_visible and hips_visible:
        # shoulders cut off — user too far down or camera too low
        _draw_arrow(frame, (w // 2, 30), direction="up",
                    label="Step forward", color=COLOR_YELLOW)
...
def _draw_arrow(
    ...
    """Draw a filled triangle arrow and label at the given tip position."""
    x, y = tip
    size = 18
    if direction == "down":
        pts = np.array([[x, y], [x - size, y - size * 2], [x + size, y - size * 2]])
        label_pos = (x - 55, y - size * 2 - 8)
    else:
        pts = np.array([[x, y], [x - size, y + size * 2], [x + size, y + size * 2]])
        label_pos = (x - 55, y + size * 2 + 20)
...
def draw_bounding_box(
    ...
    if dtw_result is None:
        color, label = COLOR_BOX_GREY, "DETECTING"
    elif dtw_result.aligned:
        color, label = COLOR_GREEN, "ALIGNED"
    else:
        color, label = COLOR_BOX_MATCHING, "MATCHING..."
...
def render(
    ...
    *,
    ...
    # positioning guide — shown while buffer is filling
